...
class VAE(nn.Module):
    def __init__(self, input_size, latent_size=15):
        ...
        self.encoder = model
        self.mu_layer = nn.Linear(in_features=self.hidden_dim, out_features=latent_size)
        self.logvar_layer = nn.Linear(in_features=self.hidden_dim, out_features=latent_size)
        ############################################################################################
        ...
        model = nn.Sequential(
            ...
            nn.Sigmoid(),
            nn.Unflatten(dim=1,unflattened_size=(1, int(np.sqrt(input_size)), int(np.sqrt(input_size))))
        )
        self.decoder = model
        ############################################################################################
        #                                      END OF YOUR CODE                                    #
...
class CVAE(nn.Module):
    def __init__(self, input_size, num_classes=10, latent_size=15):
        ...
        model = nn.Sequential(
            nn.Flatten(start_dim=1),
            nn.Linear(in_features=input_size+num_classes,out_features=self.hidden_dim),
            nn.ReLU(),
            nn.Linear(in_features=self.hidden_dim,out_features=self.hidden_dim),
            nn.ReLU(),
            nn.Linear(in_features=self.hidden_dim,out_features=self.hidden_dim),
        ...
        # Replace "pass" statement with your code
        model = nn.Sequential(
            nn.Linear(in_features=latent_size+num_classes,out_features=self.hidden_dim),
            nn.ReLU(),
            nn.Linear(in_features=self.hidden_dim,out_features=self.hidden_dim),
            nn.ReLU(),
            nn.Linear(in_features=self.hidden_dim,out_features=self.hidden_dim),
    ...
    def forward(self, x, c):
        ...
        concat = torch.cat((x_flattened,c),dim=1)
...
def reparametrize(mu, logvar):
    ...
    z = torch.exp(1/2*logvar).to('cuda')*torch.normal(0.0,1.0,size=mu.shape).to('cuda') + mu
    ################################################################################################
    #                              END OF YOUR CODE                                                #
    ################################################################################################
    return z
...
def loss_function(x_hat, x, mu, logvar):
    ...
    ################################################################################################
    # Replace "pass" statement with your code
    reconstruction_loss = nn.functional.binary_cross_entropy(input=x_hat,target=x,reduction='sum')
    sum_term = (1 + logvar - mu**2 - torch.exp(logvar))
    sum = torch.sum(sum_term)*-1/2
