...
def discriminator_loss(logits_real, logits_fake):
    ...
    true_loss = bce_loss(logits_real,torch.ones(logits_real.shape[0]).type(dtype))
    fake_loss = bce_loss(logits_fake,torch.zeros(logits_fake.shape[0]).type(dtype))
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    true_loss = 1/2 * torch.sum(torch.square(scores_real-1)) / scores_real.shape[0]
    fake_loss = 1/2 * torch.sum(torch.square(scores_fake)) / scores_fake.shape[0]
    loss = true_loss + fake_loss
...
def ls_generator_loss(scores_fake):
    ...
    loss = 1/2 * torch.sum(torch.square(scores_fake-1)) / scores_fake.shape[0]
